...
By default, M365Bridge stores the AES key for cached refresh tokens in the clear at `data/tokens/encryption.key`. Setting a master passphrase wraps that key under a passphrase-derived key instead, via an optional `docker-compose.secrets.yml` Compose override. See [Encrypting cached tokens at rest](docs/token-encryption.md) for setup and details.
...
- Optionally wrap that key under a master passphrase instead of storing it in the clear — see [Encrypting cached tokens at rest](docs/token-encryption.md)
